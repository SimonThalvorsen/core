...
- Reduced SSL/TLS shutdowns on bad networks (CFE-3023)
...
- Allowed dots in variable identifiers with no such bundle
  ...
  valid. (CFE-1915)
- Allowed requiring TLS 1.3 as the minimum version
...
- Corrected log level for `data_readstringarray*` (CFE-2922)
  ...
  bundle agent main
  {
    vars:
      "myvar"
        string => "example",
        meta => {"os[linux]"};
      "matches" slist => variablesmatching(".*", "os\[linux\]");
...
- Allowed function calls in promiser using universal "with" attribute
  (CFE-1092)
...
- Allowed opening symlinks owned by root or by the current user
  (CFE-2516)
...
- Allowed string_head and string_tail to take negative arguments
...
- LogDebug(): implement module-based debug logging.
  Now most DEBUG messages are _not_ printed even when "-d" is in use, but
...
- Rewrite iteration engine to avoid combinatorial explosion with nested variable expansions.
  ...
  One way is to always _guard_ the execution of functions to avoid
  ...
  but to check returned _type_ for CF_DATA_TYPE_NONE.
  Fixed what I could find as wrong API uses. (CFE-2162)
- Allowed arbitrary service policies (CFE-2402)
...
- Changed: Disable http TRACE method
- Changed: Verify transfered files during policy update
- Allowed getvariablemetatags() and getclassmetatags() to get a specific tag key
- Changed: Use more restrictive unix socket perms (ENT-2705)
...
- Changed: Enable agent component management policy on systemd hosts
  (CFE-2429)
- Changed: Switch processes restart_class logging to verbose
- Changed: Log level for keeping verbatim JSON to DEBUG (CFE-2141)
- Changed: Require network before CFEngine services (CFE-2435)
...
- Allowed ifelse(FALSE, $(x), "something else") to work. (CFE-2260)
...
- Changed: cronjob bundle tolerates different spacing
- Allowed editing fields in lines longer than 4k (CFE-2438)
- Don't send empty emails for logs where everything is filtered.
  (ENT-2739)
- Allowed maplist(), maparray(), and mapdata() to evaluate function calls during iteration
  (ARCHIVE-1619)
- insert_lines is no longer implicitly matching EOF as
  end of the region if `select_end` pattern is not matched . (CFE-2263)
- Changed: Remove executable bit from systemd units (CFE-2436)
...
- Changed: Split systemd units (CFE-2278)
...
- Allowed `=` in symbolic modes (Redmine #7826)
- Added: FreeBSD ports package module
...
- New language functions: processexists() and findprocesses()
  (Redmine #7633)
- Implemented new regex_replace() function. (Redmine #7346)
...
- Added: nimclient package module for AIX
...
- Added: Memory information to host info report (Jira CFE-1177)
...
- Added: file_make_mustache bundle to render mustache templates
...
- Added: printfile bodies to stdlib
- Added: New results classes body [] (Redmine #7418, #7481)
- Implemented cf-runagent --remote-bundles and cf-serverd "bundle" access promise.
  (Redmine #7581)
...
- It's now possible to reference variables in inline JSON,
  ...
  (Redmine #7871)
- Allowed inline JSON to be used in the following function
  calls:
  - data_expand()
  - difference()
  - every()
...
- Changed: classesmatching(): order of classes changed
- Changed: getindices(), getvalues(), variablesmatching(), maparray():
  order of variables returned has changed
- Changed: set_quoted_values uses bundle scoped classes
- Changed: set_config_values uses bundle scoped classes
- Changed: set_variable_values uses bundle scoped classes
- Changed: set_config_values_matching uses bundle scoped classes
- Changed: manage_variable_values_ini uses bundle scoped classes
- Changed: set_line_based should use bundle scoped classes
  (Jira CFE-1959)
- getvalues() will now return a list also for data containers,
  and will descend recursively into the containers. (Redmine #7116)
- Changed: Improve git drop user support
...
- Changed: returnszero() no longer outputs the output of a command.
  The output can be seen by enabling info mode (-I).
- cfruncommand is not executed under shell. (Redmine #7409)
- Removed: Apache CGI module
- Changed: Make maxbytes arg of readjson() and readyaml() optional
...
- Removed: Support for email settings from augments_file (Redmine #7682)
- Changed: set_variable_values_ini uses bundle scoped classes
...
- Changed: Policy files specified in the "inputs" section of
...
- Changed: Separate binary details from policy update (Redmine #7662)
- Added guard for binary upgrade during bootstrap (Redmine #7861)
- Changed: Modernize pkg module and package_method
- Removed: Userdir apache module
...
- Fixed: Problem with git not dropping privileges soon enough
- Allowed def.json up to 5MB instead of 4K.
...
- Changed: Tighten Enterprise hub permissions (Jira ENT-2708)
...
- cf-runagent now properly supports multiple -D or -s arguments
  (Redmine #7191)
- Fixed: Work around impaired class definition from augments
  (Jira CFE-2333)
...
- Changed: Suppress standard services noise on SUSE (Redmine #6968)
- Reduced verbosity of yum package module (Redmine #7485)
...
- Reduced verbosity of apt_get package module (Redmine #7485)
...
- Fixed: CFEngine choking on standard services (Jira CFE-2086)
- Fixed: cf-upgrade on SUSE
- Fixed: Stop CFEngine choking on systemctl output (Jira CFE-2806)
...
- Upgraded CFEngine dependencies to the following versions:
  ...
  - OpenLDAP 2.4.43
  - libcurl 7.46.0
- Upgraded LMDB to version 0.9.17. (Redmine #7879)
...
- Added: New results classes body (Redmine #7418)
- Added: Debug reports in cfe_internal_cleanup_agent_reports
- Added: Path to svcprop in stdlib
- Added: masterfiles-stage script to contrib
- Whitespace is now allowed in class expressions for
  readability, between class names and operators. (Redmine #7152)

Changes:

- Changed: Clarify bootstrap/failsafe reports
- Changed: Improve in-line docs for internal log maintenance
- Changed: Improve efficiency and debug reports (Redmine #7527)
- Removed: 3.5 support from masterfiles policy framework
- Long promiser strings with multiple lines are now
  abbreviated in logs. (Redmine #3964)
- Changed: Reunify Version based policy split
- Changed: Separate binary details from policy update (Redmine #7662)
- Remove /var/cfengine/cf3.<host>.runlog. (Redmine #6957)
- Changed: sys.libdir and sys.local_libdir to non version specific path
...
- Fixed: Policy errors for 3.5 and 3.6
...
- Reduced malloc() thread contention on heavily loaded
...
- Included latest security updates for dependencies.
...
- Fixed: Solaris packages no longer contain duplicate library
...
- Directories should no more be changed randomly
  into files. (Redmine #6027)
- Included latest security updates for 3.7.
- Reduced malloc() thread contention on heavily loaded
  cf-serverd, by not exiting early in the logging function, if no message
...
- Upgraded all dependencies for patch release.
- Fixed a bug which caused daemons not to be restarted on
  upgrade. (Redmine #7528)
...
- Ability to expand JSON variables directory in Mustache templates:
  ...
  expansion.
- Ability to expand the iteration _key_ in Mustache templates with @
...
- Inline JSON in policy files: surrounding with parsejson() is now
  optional _when creating a new data container_.
...
- Implemented new TLS-relevant options (Redmine #6883):
...
- Disabled class set on reverse lookup of interfaces IP addresses.
...
- Reduced verbosity of harmless errors related to socket timeouts and
...
- Relocated acls related to report collection from bundle server
  ...
  report_access_rules
- Re-organized cfe_internal splitting core from enterprise specific
...
- Move hub specific bundles from lib/VER/cfe_internal.cf into
  ...
  if set.
- Re-organized lib/VER/stdlib.cf from lists into classic array for use
  with getvalues
...
- Reduced write count activity to /var partition (Redmine #6523)
...
- Allowed copying of dead symbolic links (Redmine #6175)
...
- Marked as removed: promise_notkept_log_include, promise_notkept_log_exclude, promise_repaired_log_include,
...
- Made discovery and loading of avahi libraries more robust
...
- Included system message in output when acl promises fail
...
- Made parser output in GCC compatible format the only supported format
...
- Corrected parsing of list variables by a command module. (Redmine #2239)
...
- Made sure "restart" and "reload" are recognized keywords in packages. (Redmine #2468)
...
- Disabled xml editing functionality when libxml2 doesn't provide necessary APIs (Redmine #1937)
...
- Made qualified arrays expand correcty (Redmine #1998, Mantis #1128)
- Corrected possible errors in tcdb files when opening
- Avoid possible db corruption when mixing read/write and cursor operations
- Allowed umask value of 002 (Redmine #2496)
...
- Included xpath in messages generated by edit_xml operations (Redmine #2057)
...
- Implemented network timeout on server side to avoid keeping stale
  connections for hours.
- XML editing capabilities. See the documentation for edit_xml
  body. Note the new dependency: libxml2.
- Implemented inheritance of local classes by bundles called using
...
- Allowed filenames up to 4096 bytes in network transfers (Redmine #1199).
...
- Do not lose hard classes in cf-serverd during policy reload
  (Mantis #1218).
- Implemented receive network timeout in cf-serverd. Prevents
...
- Made network timeout for network reads configurable. Previously
...
- Disabled --parse-tree option, not ready for the release (Mantis #1063).
...
- Allowed defining arrays from modules.
- Allowed both process_stop' and signals' constraints in
...
- Major cleanup of database handling code. Should radically decrease
  ...
  _WARNING_: Berkeley DB and SQLite backends are _removed_, use
...
- Allowed ignore_missing_bundles to affect methods: bundles too
