...
- IPv6-only interfaces added to sys.hardware\_(addresses,mac) (CFE-3164)
...
- Added IPv6 hard classes with the "ipv6\_" prefix (CFE-2310)
...
- When a bodydefault:<promise*type>*<body_type> body is
...
- In a services promise, if the service*method bundle is not specified,
  it defaults to the promiser string (canonified) with "service*" as a
...
- New in `access` promises for `bundle server access_rules`
  ...
  - For these new attributes, regular expressions
    are not allowed, only CIDR notation for "admit/deny*ips", exact
...
- Files created via log\_\* actions now have mode 600 (Redmine #1578)
...
- Running in Solaris zone is now detected and classes "zone" and
  "zone\_<name>" are created in this case.
...
- mac\_<mac_address>:: - discovered MAC addresses
